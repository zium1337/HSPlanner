...
pub struct DamageFormulaDto { pub base: f64, pub per_level: f64 }
impl From<DamageFormulaDto> for calc::DamageFormula {
    fn from(v: DamageFormulaDto) -> Self {
        Self { base: v.base, per_level: v.per_level }
    }
}

#[derive(Deserialize)]
pub struct DamageRowDto { pub min: f64, pub max: f64 }
impl From<DamageRowDto> for calc::DamageRow {
    fn from(v: DamageRowDto) -> Self { Self { min: v.min, max: v.max } }
}
    ...
    fn from(v: BonusSourceDto) -> Self {
        match v {
            BonusSourceDto::AttributePoint { source, value } =>
                calc::BonusSource::AttributePoint { source: norm(&source), value },
            BonusSourceDto::SkillLevel { source, value } =>
                calc::BonusSource::SkillLevel { source: norm(&source), value },
        }
    }
}
...
pub struct SkillDto {
    ...
    #[serde(default)] pub damage_type: Option<String>,
    #[serde(default)] pub damage_formula: Option<DamageFormulaDto>,
    #[serde(default)] pub damage_per_rank: Option<Vec<DamageRowDto>>,
    #[serde(default)] pub bonus_sources: Vec<BonusSourceDto>,
}
impl From<SkillDto> for calc::Skill {
    fn from(v: SkillDto) -> Self {
        calc::Skill {
            ...
            damage_per_rank: v.damage_per_rank.map(|t| t.into_iter().map(Into::into).collect()),
...
impl From<WeaponDto> for calc::Weapon {
    fn from(v: WeaponDto) -> Self {
        calc::Weapon { name: v.name, damage_min: v.damage_min, damage_max: v.damage_max }
    }
}
...
pub struct SkillDamageInput {
    ...
    #[serde(default)] pub attributes: HashMap<String, NumberOrRange>,
    #[serde(default)] pub stats: HashMap<String, NumberOrRange>,
    #[serde(default)] pub skill_ranks_by_name: HashMap<String, f64>,
    #[serde(default)] pub item_skill_bonuses: HashMap<String, (f64, f64)>,
    #[serde(default)] pub enemy_conditions: HashMap<String, bool>,
    #[serde(default)] pub enemy_resistances: HashMap<String, f64>,
    #[serde(default)] pub skills_by_name: HashMap<String, SkillDto>,
    #[serde(default = "default_projectile_count")] pub projectile_count: u32,
}
...
pub struct WeaponDamageInput {
    #[serde(default)] pub weapon: Option<WeaponDto>,
    #[serde(default)] pub stats: HashMap<String, NumberOrRange>,
    #[serde(default)] pub enemy_conditions: HashMap<String, bool>,
}

#[derive(Serialize)]
pub struct ExtraSourceOut { pub label: String, pub pct: f64 }
...
impl From<calc::SkillDamageBreakdown> for SkillDamageOutput {
    fn from(v: calc::SkillDamageBreakdown) -> Self {
        Self {
            effective_rank_min: v.effective_rank_min, effective_rank_max: v.effective_rank_max,
            base_min: v.base_min, base_max: v.base_max,
            flat_min: v.flat_min, flat_max: v.flat_max,
            synergy_min_pct: v.synergy_min_pct, synergy_max_pct: v.synergy_max_pct,
            ...
            extra_damage_sources: v.extra_damage_sources.into_iter()
                .map(|e| ExtraSourceOut { label: e.label.to_string(), pct: e.pct })
                .collect(),
            ...
            hit_min: v.hit_min, hit_max: v.hit_max,
            crit_min: v.crit_min, crit_max: v.crit_max,
            final_min: v.final_min, final_max: v.final_max,
            avg_min: v.avg_min, avg_max: v.avg_max,
        }
    }
}
...
impl From<calc::WeaponDamageBreakdown> for WeaponDamageOutput {
    fn from(v: calc::WeaponDamageBreakdown) -> Self {
        Self {
            ...
            weapon_damage_min: v.weapon_damage_min, weapon_damage_max: v.weapon_damage_max,
            ...
            crit_min: v.crit_min, crit_max: v.crit_max,
            avg_min: v.avg_min, avg_max: v.avg_max,
            dps_min: v.dps_min, dps_max: v.dps_max,
        }
    }
}
...
pub fn compute_skill_damage(input: SkillDamageInput) -> Option<SkillDamageOutput> {
    ...
    let skills_by_name: HashMap<String, calc::Skill> = input.skills_by_name
        .into_iter()
